...
function escQ(s){return(s||'').toString().replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/"/g,'\\"');}
...
function startMandatoryTracking(){
  ...
  // We use watchPosition to get updates, plus a 5 min interval fallback to push to DB
  let lastLat=0,lastLng=0;
  
  const pushLocation=async()=>{
    if(!lastLat||!S.salesman)return;
    const{error}=await db.from('salesmen').select('id').eq('id',S.salesman.id).single();
    if(error)return; // DB check passed? Actually just upsert to salesman_locations
  ...
  gpsWatchId = navigator.geolocation.watchPosition(
    pos=>{
      ...
      const ind=document.getElementById('gps-indicator');if(ind)ind.style.display='inline-block';
      pushLocation(); // Push on change
    },
    err=>{
      console.warn('GPS Error:',err);
      const ind=document.getElementById('gps-indicator');if(ind){ind.style.display='inline-block';ind.style.background='#ef4444';ind.textContent='📍 GPS Off';}
    },
    {enableHighAccuracy:true,timeout:10000,maximumAge:0}
  );
  
  // Also push every 5 mins to ensure freshness in DB even if stationary
  S.gpsInterval=setInterval(pushLocation,5*60*1000);
}
...
async function placeOrder(){
  ...
  try{
    ...
    setTimeout(()=>{window.open(`https://wa.me/${WA_NUM}?text=${msg}`,'_blank');btn.innerHTML='Place Order &amp; Send on WhatsApp';btn.disabled=false;gotoPage('pg-order-history');},600);
...
// ============================================================
//   DELIVERY MODAL — mark delivered, verify bill against paper bill,
//   change payment terms if needed, optionally record collection
// ============================================================
async function openDeliveryModal(orderId){
  const{data:o,error}=await db.from('orders').select('*, retailers(id,name,outstanding,credit_limit), order_items(id,quantity,rate,bonus_quantity,products(name,sku))').eq('id',orderId).single();
  if(error||!o){toast('Could not load order');return;}
  const items=o.order_items||[];
  const tot=items.reduce((s,i)=>s+((Number(i.quantity)||0)*(Number(i.rate)||0)),0);
  ...
  document.getElementById('modal-body').innerHTML=`
    <div class="card" style="background:var(--or-grad-soft);border:.5px solid #fed7aa;padding:14px;margin-bottom:14px">
      ...
      <div style="font-size:12px;color:var(--tx-2);margin-top:6px">Order: <strong>${fmtOrd(o.order_number)}</strong> · Original Bill: <strong style="color:var(--or)">${fmtMoney(tot)}</strong></div>
    </div>

    <!-- BILL VERIFICATION SECTION -->
    <div style="background:#fffbea;border:0.5px solid #fcd34d;border-radius:14px;padding:12px;margin-bottom:14px">
      <div style="display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700;color:#78350f;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">
        ${ico('check',14)} Tick Items Actually Delivered
      </div>
      <div style="font-size:12px;color:#78350f;margin-bottom:10px;font-weight:500;line-height:1.5">Untick any item the retailer refused or you couldn't deliver. The delivered total below will update automatically.</div>
      <div id="dlv-items-list">
        ${items.map((i,idx)=>{const lineTot=(Number(i.quantity)||0)*(Number(i.rate)||0);return`<label style="display:flex;align-items:flex-start;gap:8px;padding:8px 0;border-top:0.5px solid rgba(180,83,9,.18);cursor:pointer">
          <input type="checkbox" class="dlv-item-check" data-idx="${idx}" data-name="${escQ(i.products?.name||'')}" data-linetot="${lineTot.toFixed(2)}" checked onchange="recalcDeliveredTotal(${tot})" style="margin-top:2px;flex-shrink:0;width:18px;height:18px;accent-color:#ea580c">
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:600;line-height:1.3">${esc(i.products?.name||'')}</div>
            <div style="font-size:11px;color:#78350f;margin-top:2px;font-weight:500">${i.quantity} pcs${i.bonus_quantity>0?' + '+i.bonus_quantity+' free':''} @ ₹${(Number(i.rate)||0).toFixed(2)} = ₹${lineTot.toFixed(2)}</div>
          </div>
        </label>`}).join('')}
      </div>
      <div style="margin-top:12px;display:flex;gap:8px">
        <button type="button" class="btn btn-ghost btn-xs" style="flex:1" onclick="dlvCheckAll(${tot})">${ico('check',12)} Check all</button>
        <button type="button" class="btn btn-ghost btn-xs" style="flex:1" onclick="dlvUncheckAll(${tot})">${ico('x',12)} Uncheck all</button>
      </div>
      ...
      <div style="margin-top:14px;padding:12px;background:#fff;border-radius:10px;border:0.5px solid rgba(180,83,9,.25);display:flex;justify-content:space-between;align-items:center">
        <div>
          ...
          <div id="dlv-rejected-note" style="font-size:11px;color:#7f1d1d;font-weight:500;margin-top:2px;display:none"></div>
        </div>
        <div id="dlv-delivered-total" style="font-family:var(--font-serif);font-size:24px;color:#ea580c;font-weight:400;letter-spacing:-0.02em">${fmtMoney(tot)}</div>
      </div>
    ...
    <div class="fg"><label>Payment Type at Delivery</label>
      ...
      <div style="font-size:11px;color:var(--mu);margin-top:5px;font-weight:500">Originally placed as <strong>${o.payment_term}</strong>${o.payment_term==='credit'&&o.credit_period_days?' ('+o.credit_period_days+' days)':''}. Change it if the retailer prefers something else now.</div>
    </div>
    ...
    <button class="btn btn-or" id="dlv-save" onclick="saveDelivery('${orderId}',${tot},'${o.retailers?.id||''}','${escQ(o.retailers?.name||'')}',${items.length})">${ico('check',16)} Confirm Delivery</button>
  `;
  setTimeout(()=>{
    ...
    // Initialize delivered total dataset
    const dtEl=document.getElementById('dlv-delivered-total');
    if(dtEl)dtEl.dataset.value=tot.toFixed(2);
  },50);
  openModal();
}

function recalcDeliveredTotal(origTotal){
  const checks=document.querySelectorAll('.dlv-item-check');
  let delivered=0,rejected=0,rejCount=0;
  checks.forEach(c=>{
    const lt=parseFloat(c.dataset.linetot)||0;
    if(c.checked)delivered+=lt;
    else{rejected+=lt;rejCount++;}
  });
  const dtEl=document.getElementById('dlv-delivered-total');
  const noteEl=document.getElementById('dlv-rejected-note');
  if(dtEl){
    dtEl.textContent=fmtMoney(delivered);
    dtEl.dataset.value=delivered.toFixed(2);
    dtEl.style.color=rejCount>0?'#7f1d1d':'#ea580c';
  }
  if(noteEl){
    if(rejCount>0){
      noteEl.style.display='block';
      noteEl.textContent=`${rejCount} item${rejCount>1?'s':''} rejected — ${fmtMoney(rejected)} deducted from original ${fmtMoney(origTotal)}`;
    }else{
      noteEl.style.display='none';
    }
  }
  // Update "full" option label and amount auto-fill
  const cnSel=document.getElementById('dlv-collect-now');
  if(cnSel){
    const fullOpt=cnSel.querySelector('option[value="full"]');
    if(fullOpt)fullOpt.textContent='Yes — full '+fmtMoney(delivered);
    if(cnSel.value==='full'){
      const amt=document.getElementById('dlv-amt');
      if(amt)amt.value=delivered.toFixed(2);
    }
  }
  // Re-run bill match if paper total entered
  checkBillMatch();
}

function dlvCheckAll(origTotal){document.querySelectorAll('.dlv-item-check').forEach(c=>c.checked=true);recalcDeliveredTotal(origTotal);}
function dlvUncheckAll(origTotal){document.querySelectorAll('.dlv-item-check').forEach(c=>c.checked=false);recalcDeliveredTotal(origTotal);}
...
function checkBillMatch(){
  ...
  // Compare paper total against the LIVE delivered total (which excludes unticked items)
  const dtEl=document.getElementById('dlv-delivered-total');
  ...
  else if(absDiff<5)el.innerHTML=`<span style="color:#78350f">⚠ Small difference: ${diff>0?'+':''}${fmtMoney(diff)} (probably rounding)</span>`;
...
async function saveDelivery(orderId,orderTotal,retailerId,retailerName,itemCount){
  ...
  // === Snapshot ALL form values BEFORE any await ===
  ...
  if(!ptEl||!cnEl){
    toast('Delivery form not loaded — please reopen and try again');
    return;
  }
  ...
  const paperTotalRaw=ptotalEl?.value;
  const paperTotal=paperTotalRaw?parseFloat(paperTotalRaw):null;

  // === Compute delivered total from ticked items ===
  const checks=document.querySelectorAll('.dlv-item-check');
  const checkedCount=Array.from(checks).filter(c=>c.checked).length;
  const totalItems=checks.length;
  const uncheckedNames=Array.from(checks).filter(c=>!c.checked).map(c=>c.dataset.name);
  const deliveredTotal=parseFloat(dtEl?.dataset?.value)||orderTotal;
  const rejectedAmount=orderTotal-deliveredTotal;

  // === Validate: zero items delivered is invalid ===
  if(totalItems>0&&checkedCount===0){
    const proceed=await ui.confirm({
      title:'No items delivered',
      message:'You haven\'t ticked any item as delivered. Do you mean to cancel this order instead?\n\nIf you proceed, the order will be marked delivered with ₹0 amount.',
      confirmText:'Mark delivered anyway',cancelText:'Go back',danger:true
    });
    if(!proceed)return;
  }

  // === Warn on partial delivery (some items unticked) ===
  if(totalItems>0&&checkedCount<totalItems&&checkedCount>0){
    const proceed=await ui.confirm({
      title:`${totalItems-checkedCount} item${totalItems-checkedCount>1?'s':''} rejected`,
      message:`These items will be REMOVED from the bill:\n\n${uncheckedNames.slice(0,5).map(n=>'• '+n).join('\n')}${uncheckedNames.length>5?'\n• ...and '+(uncheckedNames.length-5)+' more':''}\n\nOriginal bill: ${fmtMoney(orderTotal)}\nDelivered total: ${fmtMoney(deliveredTotal)}\nReduction: ${fmtMoney(rejectedAmount)}\n\nProceed?`,
      confirmText:'Yes, deliver partial',cancelText:'Go back',danger:false
    });
    if(!proceed)return;
  }

  // === Warn on significant paper bill mismatch (compared to delivered total) ===
  let billMismatchNote='';
  if(paperTotal!==null&&!isNaN(paperTotal)){
    const diff=paperTotal-deliveredTotal;
    if(Math.abs(diff)>=5){
      const proceed=await ui.confirm({
        title:'Paper bill does not match delivered total',
        message:`Delivered total: ${fmtMoney(deliveredTotal)}\nPaper bill: ${fmtMoney(paperTotal)}\nDifference: ${diff>0?'+':''}${fmtMoney(diff)}\n\nMark delivered anyway? Admin will be flagged for review.`,
        confirmText:'Deliver & flag for admin',cancelText:'Go back',danger:true
      });
      if(!proceed)return;
      billMismatchNote=`BILL MISMATCH: delivered ${fmtMoney(deliveredTotal)} vs paper ${fmtMoney(paperTotal)} (diff ${fmtMoney(diff)})`;
    }
  }
  ...
  try{
    // Step 1: mark order delivered (keep original items intact, store delivered_total as a note)
    const{error:e1}=await db.from('orders').update({status:'delivered',payment_term:newPT,credit_period_days:newCD}).eq('id',orderId);
    if(e1)throw e1;

    // Step 2: audit log — captures the rejected items and delivered total
    let auditSummary=`Delivered as ${newPT}.`;
    if(rejectedAmount>0)auditSummary+=` Partial: ${fmtMoney(deliveredTotal)} of ${fmtMoney(orderTotal)} (${totalItems-checkedCount} item${totalItems-checkedCount>1?'s':''} rejected, ${fmtMoney(rejectedAmount)} reduction).`;
    if(billMismatchNote)auditSummary+=' '+billMismatchNote;
    db.from('order_edits').insert({
      order_id:orderId,
      edited_by_type:'salesman',edited_by_id:S.salesman?.id,edited_by_name:S.salesman?.name||'?',
      edit_summary:auditSummary,
      after_snapshot:{
        status:'delivered',payment_term:newPT,credit_period_days:newCD,
        original_total:orderTotal,delivered_total:deliveredTotal,rejected_amount:rejectedAmount,
        items_delivered:checkedCount,items_total:totalItems,
        unchecked_items:uncheckedNames,
        paper_bill_total:paperTotal,
        mismatch:!!billMismatchNote
      }
    }).then(({error})=>{if(error)console.warn('audit log failed:',error.message);});

    // Step 3: handle retailer outstanding adjustment for partial delivery on credit orders
    // If the order goes to credit, the retailer's outstanding has likely already been adjusted by
    // the original amount. If we're delivering less, we need to credit back the rejected amount.
    // (This is safe because the original order amount was added when the order was placed.)
    if(rejectedAmount>0.01&&retailerId&&newPT==='credit'){
      // Reduce outstanding by the rejected amount (since less was actually delivered)
      const{data:retData}=await db.from('retailers').select('outstanding').eq('id',retailerId).single();
      const currentOutstanding=Number(retData?.outstanding||0);
      const{error:eOut}=await db.from('retailers').update({outstanding:Math.max(0,currentOutstanding-rejectedAmount)}).eq('id',retailerId);
      if(eOut)console.warn('outstanding adjust failed:',eOut.message);
    }

    // Step 4: optional collection — use the DELIVERED total, not original
    if(collectNow!=='no'&&retailerId&&amtVal>0){
      const{data:ordResp}=await db.from('orders').select('order_number').eq('id',orderId).single();
      const noteSuffix=rejectedAmount>0?` (partial: ${fmtMoney(deliveredTotal)} of ${fmtMoney(orderTotal)})`:'';
      const{error:e2}=await db.from('collections').insert({
        retailer_id:retailerId,salesman_id:S.salesman.id,
        amount:amtVal,mode:modeVal,reference:refVal,
        notes:'At delivery of '+fmtOrd(ordResp?.order_number||0)+noteSuffix
      });
      if(e2)throw e2;
      // Mark paid if collected = delivered total (within 1₹ tolerance)
      if(amtVal>=deliveredTotal-0.01){
        const{error:e3}=await db.from('payments').update({status:'paid',paid_on:toYMD(new Date())}).eq('order_id',orderId);
        if(e3)console.warn('mark paid failed:',e3.message);
      }
    }

    let toastMsg='Order delivered';
    if(rejectedAmount>0)toastMsg+=` (${fmtMoney(deliveredTotal)} of ${fmtMoney(orderTotal)})`;
    if(collectNow!=='no'&&amtVal>0)toastMsg+=' & payment recorded';
    if(billMismatchNote)toastMsg+=' (flagged)';
    toast(toastMsg);
    closeModal();
    await loadSalesmanHome();
...
function statusLabel(s){return{placed:'Placed',confirmed:'Confirmed',dispatched:'Dispatched',delivered:'Delivered',done:'Done',cancelled:'Cancelled'}[s]||s;}
...
function adminLogin(){if(document.getElementById('admin-pw').value===ADMIN_PW){S.isAdmin=true;persistState();gotoPage('pg-admin');loadAdminDashboard();}else toast('Wrong password');}
function adminLogout(){document.getElementById('admin-pw').value='';S.isAdmin=false;persistState();gotoPage('pg-landing');}
...
function renderAdminOrders(orders){
  ...
  const cardHtml=o=>{
    const tot=(o.order_items||[]).reduce((s,i)=>s+(i.quantity*(i.rate||0)),0);
    const status=o.status||'placed';
    return`<div class="oc" onclick="openOrderDetail('${o.id}')" style="cursor:pointer">
      <div class="oh">
...
async function openOrderDetail(orderId){
  ...
  document.getElementById('modal-body').innerHTML=`
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      ${status!=='delivered'&&status!=='cancelled'?`<button class="btn btn-ghost btn-sm" style="border:1px solid var(--bd)" onclick="editOrder('${o.id}');closeModal();">${ico('edit',13)} Edit</button>`:''}
...
async function loadReports(){
  ...
  const{data}=await db.from('orders').select('order_date, status, salesmen(id,name), retailers(id,name), payments(status), order_items(quantity,rate)').order('order_date',{ascending:false});
...
async function loadAdminMap(){
  ...
  if(data.length>0) { adminMap.fitBounds(bounds, {padding: [30, 30], maxZoom:15}); }
}
...
function togglePredictiveForecast() {
